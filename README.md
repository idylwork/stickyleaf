...
マークダウンと複数タブに対応した Tauri 製の半常駐用メモアプリ。
...
{{ /is_shown }}
```
#### 繰り返し `{{ #example }})` `{{ /example }})`
条件分岐と同じですが、コンマが含まれる場合にコンマ区切りで文字列を分割して繰り返します。
構文の内部でプレースホルダを使うと、分割された文字列のみ表示されます。
```
{{ #name }}
- {{ name }}
{{ /name }}
...
#### 否定条件分岐 `{{ ^example }})` `{{ /example }})`
文字列が空のときのみ中の文字列がが表示されます。
